    ...
    const HandleSubmit = async (e) => {
        ...
        if (Object.keys(isError).length === 0) {
            ...
            try {
                ...
                if (res.data === 'email already present' || res.data === 'error occured at server side') {
                ...
                <div className='sign-in-con'>
                    ...
                    <form onSubmit={(e) => { HandleSubmit(e) }}>
                        ...
                        <div className='field'>
                            ...
                            <input type='text' name='name' value={name} id='sign-up-name' onChange={(e) => { setName(e.target.value) }} />
                            <i className='error-massage'>{formSubmit && formerror.name}</i>
                        </div>

                        <div className='field'>
                            <p className='lable'>Email address  :-</p>
                            <input type={'email'} name='email' value={email} id='sign-up-email' onChange={(e) => { setEmail(e.target.value) }} />
                            <i className='error-massage'>{formSubmit && formerror.email}</i>
                        </div>

                        <div className='field'>
                            <p className='lable'>Mobile number :-</p>
                            <input type={'text'} name='m_number' value={m_number} id='sign-up-number' onChange={(e) => { setNumber(e.target.value) }} />
                            <i className='error-massage'>{formSubmit && formerror.m_number}</i>
                        </div>

                        <div className='field'>
                            <p className='lable'>Password :-</p>
                            <input type={passwordShown ? "text" : "password"} name='password' value={password} id='sign-up-password' onChange={(e) => { setPassword(e.target.value) }} />
