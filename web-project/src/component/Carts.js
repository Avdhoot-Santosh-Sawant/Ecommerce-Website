...
export default function Carts(props) {
    ...
    const deleteCart = async (pID) => {
        ...
        if (localStorage.getItem('cartData') === 'undefined' || localStorage.getItem('cartData') == null || localStorage.getItem('cartData').length === 0) {
    ...
    const cartData = () => {
        ...
        if (window.localStorage.getItem('cartData') == null || JSON.parse(localStorage.getItem('cartData')).length === 0) {
    ...
    const handlePayment = async () => {
        ...
        if(auth){   
            ...
            if(authData.name && authData.mobile && authData.email){
                ...
                console.log('sorry, login data fetch error')
            }
        }
        else{
    ...
    return (
        <>
            ...
            <div className="container mt-3">
                ...
                <h3 style={{ fontWeight: "700", display: "inline-block", paddingLeft: '20px' }}>Cart List </h3>
                <div style={{ border: "1px solid black" }}></div>
            </div>

            <div id="empty-icon">
                <img alt="..." src="/images/emptycart2.gif"/>
                ...
                <div className="cart-con" >
                    {
                        productsCart.map((pro) => {
                            return <Cart key={pro.id} product={pro} deleteCart={deleteCart} updateQun={updateQun} />
                        })
                    }
                </div>
            ...
            <div className="container cart-total-buy" id="cart-total-buy">
                ...
                <div><button id='buy-btn' onClick={handlePayment}>Buy now</button></div>
